...
public class CollectivityTransactionRepository {
    ...
    public void save(CollectivityTransaction tx) {
        String sql = "INSERT INTO collectivity_transaction (collectivity_id, creation_date, amount, payment_mode, account_credited_id, member_debited_id) VALUES (?, ?, ?, ?, ?, ?)";
        try (Connection conn = DataSource.getConnection();
             PreparedStatement ps = conn.prepareStatement(sql)) {
            ps.setInt(1, Integer.parseInt(tx.getCollectivityId()));
            ps.setDate(2, Date.valueOf(tx.getCreationDate()));
            ps.setDouble(3, tx.getAmount());
            ps.setString(4, tx.getPaymentMode().name());
            ps.setInt(5, Integer.parseInt(tx.getAccountCreditedId()));
            ps.setInt(6, Integer.parseInt(tx.getMemberDebitedId()));
            ps.executeUpdate();
    ...
    public List<CollectivityTransaction> findByCollectivityIdAndDateBetween(String collectivityId, LocalDate from, LocalDate to) {
        ...
        try (Connection conn = DataSource.getConnection();
            ...
            while (rs.next()) {
                ...
                tx.setCollectivityId(String.valueOf(rs.getInt("collectivity_id")));
